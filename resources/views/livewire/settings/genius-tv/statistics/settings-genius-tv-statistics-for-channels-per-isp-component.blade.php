        ...
        <div class="h-96 overflow-y-auto">
            <x-table :headers="$headers" :rows="$nanguIsps" with-pagination>
                @scope('cell_actions', $nanguIsp)
                    <x-button wire:click='downloadChannelsMonthlyUsageReport({{ $nanguIsp->id }})' icon="o-arrow-down-circle" class="btn-circle bg-transparent border-none btn-sm text-blue-500 shadow-none" />
                @endscope
            </x-table>
        </div>
