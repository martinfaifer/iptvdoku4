<div>
    <x-share.cards.base-card title="">
        ...
        <div>
            <x-table :headers="$headers" :rows="$gpus" with-pagination>
                @scope('cell_actions', $gpu)
                    <div class="flex mx-auto gap-4">
                        <button class="btn btn-sm btn-circle bg-opacity-0 border-transparent shadow-none"
                            wire:click="edit({{ $gpu->id }})">
                            <x-heroicon-o-pencil class="w-4 h-4 text-green-500" />
                        </button>
                        <button class="btn btn-sm btn-circle bg-opacity-0 border-transparent shadow-none"
                            wire:click="destroy({{ $gpu->id }})" wire:confirm="Opravdu odebrat?">
        ...
        <x-form wire:submit="create">
            ...
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12 ">
                    <x-input label="Model" wire:model="createForm.name" />
                </div>
                <div class="col-span-12 ">
                    <x-input type="number" label="Maximální počet streamů" wire:model="createForm.max_streams" />
                </div>
            </div>
            {{-- action section --}}
    ...
    <x-modal wire:model="editModal" persistent class="modal-bottom sm:modal-middle fixed">
        <x-form wire:submit="update">
            ...
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12">
                    <x-input label="Model" wire:model="updateForm.name" />
                </div>
                <div class="col-span-12 ">
                    <x-input type="number" label="Maximální počet streamů" wire:model="updateForm.max_streams" />
                </div>
            </div>
            {{-- action section --}}
            <div class="flex justify-between">
