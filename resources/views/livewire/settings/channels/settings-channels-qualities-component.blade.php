<div>
    <x-share.cards.base-card title="">
        ...
        <div>
            <x-table :headers="$headers" :rows="$qualities" with-pagination>
                @scope('cell_actions', $quality)
                    <div class="flex mx-auto gap-4">
                        <button class="btn btn-sm btn-circle bg-opacity-0 border-transparent"
                            wire:click="destroy({{ $quality->id }})" wire:confirm="Opravdu odebrat?">
        ...
        <x-form wire:submit="create">
            ...
            <div class="grid grid-cols-12 gap-4">
                <div class="col-span-12">
                    <x-input label="Rozlišení (šířka)" wire:model="form.name" required />
                </div>
                <div class="col-span-12">
                    <x-input label="Očekáváný bitrate" wire:model="form.bitrate" />
                </div>
                <div class="col-span-12">
                    <x-choices-offline label="Formát" wire:model="form.format" :options="$formats" single clearable />
                </div>
            </div>
