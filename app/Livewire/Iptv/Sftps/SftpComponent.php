...
class SftpComponent extends Component
{
    ...
    public function mount(mixed $sftpServer = null): void
    {
        if (!blank($sftpServer)) {
            if (!$sftpServerModel = SftpServer::where('id', $sftpServer)->first()) {
                $this->redirect('/sat-cards', true);
            } else {
                $this->sftpServer = $sftpServerModel;
            }
        } else {
